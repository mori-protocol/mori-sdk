...
contract MoriV3LmPool is IMoriV3LmPool {
    ...
    IMoriV3Pool public immutable pool;
    ...
    modifier onlyPoolOrMasterChef() {
        require(msg.sender == address(pool) || msg.sender == address(masterChef), 'Not pool or MCM');
        _;
    }
        ...
        if (lmLiquidity != 0) {
            for (uint256 i = 1; i <= idLength; i++) {
                ...
                if (rewarder != address(0)) {
                    (uint256 rewardPerSecond, uint256 endTime) = masterChef.getLatestPeriodInfo(
                        address(pool),
                        rewarder
                    );
    ...
    function crossLmTick(int24 tick, bool zeroForOne) external override onlyPool {
        ...
        int128 lmLiquidityNet = lmTicks.cross(tick, idLength, rewarderIds, rewardsGrowthGlobalX128);
    ...
    function getRewardGrowthInside(
        ...
        (, int24 tick, , , , , ) = pool.slot0();
        return lmTicks.getRewardGrowthInside(tickLower, tickUpper, tick, rewarder, rewardsGrowthGlobalX128[rewarder]);
    }
}
