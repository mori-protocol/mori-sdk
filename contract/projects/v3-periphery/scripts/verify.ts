...
async function main() {
  ...
  const deployedContracts_v3_core = await import(`@moriswap/v3-core/deployments/${networkName}.json`)
  const deployedContracts_v3_periphery = await import(`@moriswap/v3-periphery/deployments/${networkName}.json`)

  // Verify swapRouter
  console.log(
    `Verify swapRouter:${deployedContracts_v3_periphery.SwapRouter} with moriV3PoolDeployer:${deployedContracts_v3_core.MoriV3PoolDeployer}, moriV3Factory:${deployedContracts_v3_core.MoriV3Factory} and wNative:${config.WNATIVE}`
  )
  await verifyContract(deployedContracts_v3_periphery.SwapRouter, [
    deployedContracts_v3_core.MoriV3PoolDeployer,
    deployedContracts_v3_core.MoriV3Factory,
    config.WNATIVE,
  ])
  await sleep(10000)
  ...
  // Verify moriInterfaceMulticall
  console.log('Verify moriInterfaceMulticall')
  await verifyContract(deployedContracts_v3_periphery.MoriInterfaceMulticall)
  await sleep(10000)
