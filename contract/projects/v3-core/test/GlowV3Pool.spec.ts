...
import { TestMoriV3SwapPay } from '../typechain-types/contracts/test/TestMoriV3SwapPay'
...
import { TestMoriV3ReentrantCallee } from '../typechain-types/contracts/test/TestMoriV3ReentrantCallee'
...
describe('MoriV3Pool', () => {
  ...
  let factory: MoriV3Factory
  let pool: MockTimeMoriV3Pool

  let swapTarget: TestMoriV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeMoriV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestMoriV3ReentrantCallee')
      ).deploy()) as TestMoriV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestMoriV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestMoriV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestMoriV3SwapPay
