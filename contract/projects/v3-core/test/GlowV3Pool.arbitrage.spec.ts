...
import { MoriV3PoolSwapTest } from '../typechain-types/contracts/test/MoriV3PoolSwapTest'
...
describe('MoriV3Pool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('MoriV3PoolSwapTest')
            const tester = (await testerFactory.deploy()) as MoriV3PoolSwapTest
