...
describe('MoriV3Factory', () => {
  let wallet: Wallet, other: Wallet

  let deployer: MoriV3PoolDeployer
  ...
  const fixture = async () => {
    ...
    const deployer_ = (await deployerFactory.deploy()) as MoriV3PoolDeployer
    const factory_ = (await factoryFactory.deploy(deployer_.address)) as MoriV3Factory

    await deployer_.setFactoryAddress(factory_.address)
    return [factory_, deployer_] as [MoriV3Factory, MoriV3PoolDeployer]
  }
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('MoriV3Pool')).bytecode
  })
  ...
  async function createAndCheckPool(
    ...
    const poolContractFactory = await ethers.getContractFactory('MoriV3Pool')
