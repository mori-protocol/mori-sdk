...
/// @title Canonical MoriSwap V3 factory
/// @notice Deploys MoriSwap V3 pools and manages ownership and control over pool protocol fees
contract MoriV3Factory is IMoriV3Factory {
    /// @inheritdoc IMoriV3Factory
    ...
    function createPool(address tokenA, address tokenB, uint24 fee) external override returns (address pool) {
        ...
        pool = IMoriV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IMoriV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IMoriV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IMoriV3Pool(pool).setLmPool(lmPool);
    }
}
